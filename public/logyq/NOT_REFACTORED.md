...
- **Suppressed SVG double-click.** The node `dblclick` bind is gone. A capture mute still swallows SVG double-clicks; do not unmute it. Phone edit is v162 pointer double-tap → `editSelected({ uid })`, or keyboard `E`.
