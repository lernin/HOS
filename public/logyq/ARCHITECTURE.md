...
- Hold (~160ms, 8px slop) latches through synthetic `mousedown`/`mousemove`/`mouseup` into existing `d3.drag()` (`shiftKey: false`). Slide past slop *before* that beat pans the map (`applyFingerPan`) and does not arm card-drag. Flick and double-tap must not start that drag. Real phone touches do not start desktop `d3.drag`.
