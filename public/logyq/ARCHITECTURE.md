...
- Phone `autoFit` scales to **full canvas width** (may zoom in) and centers in the band below the header. In landscape the header is a side rail, the canvas is already inset by that rail, and a tall narrow header is not treated as a top inset (`sideRail` → `headerH` 0). Desktop still `Math.min(1, width, height)`.
