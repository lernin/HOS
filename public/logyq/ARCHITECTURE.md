...
- Hold-drag auto-pan is `LOGYQPreview.gestures.edgePan` via `centerPanVector` (offset from viewport center, `PAN_DEAD_PX` 56 / `PAN_STEP` 16) then `clampPanToContent` (~½ card overlap). Detect from the **finger**; feed drop mousemove at the **visual** point. Do not restore screen-edge bands.
