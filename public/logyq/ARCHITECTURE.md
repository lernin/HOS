...
- Finger hold-drag preview is an SVG **clone of only the held card**. On latch, `liftPx()` (1.45cm) pans the map north **and** offsets the floating card north by the same D (`fingerOffset` `{0, -D}`). Side offset stays 0. Set `LATCH_MAP_SHIFT: false` to skip the map pan. Drop/cancel/yield reverts the latch shift.
