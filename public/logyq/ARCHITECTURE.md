...
- Hold-drag auto-pan is `LOGYQPreview.gestures.edgePan` via `centerPanVector` (offset from viewport center, `PAN_DEAD_PX` 56 / `PAN_STEP` 16) then `clampPanToContent` (~½ card inset from the **leading** viewport edge). Detect from the **finger**; feed drop mousemove at the **visual** point. Do not restore screen-edge bands. Do not pin the tree to the trailing edge.
