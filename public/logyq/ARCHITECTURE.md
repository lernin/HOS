...
- Hold (~160ms, 8px slop) latches through synthetic `mousedown`/`mousemove`/`mouseup` into existing `d3.drag()` (`shiftKey: false`). Phone zoom is allowed to start on a card; slide past slop before 160ms keeps that same `d3.zoom` pan and cancels the hold. A still latch calls `stopZoomGesture` then lifts the card. Flick and double-tap must not start that drag.
