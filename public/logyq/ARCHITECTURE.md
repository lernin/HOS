...
- Direct flick is one recognizer → direction bucket → `createRelative`. Down/left/right/up share the same calm settle (no editor, no camera fly, select the new uid, `immediate` snap to reserved `d.x,d.y` slots). Do not `interrupt()` a 260ms sibling tween without snapping — that stacked the new blank on the unmoved row. Do not arm `#logyq-v162-action` on flick; nursery/recording is later. Header mic remains a separate voice path into the type-or-speak field. Double-tap / selection / focus key off `_uid` only (`editSelected({ uid })`, `nodeUid` from `__data__.data._uid`). Never resolve the edit target by `name` / label / `cardText`. Blank labels may repeat; `selectByName('')` is rejected.
