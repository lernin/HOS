...
- **Wrong-card editor after flick-create.** Two leaks, not a name collision. (1) `createRelative` took `state.selectedUid` as the parent. Flick did `selectByUid(candidate.uid)` first, but after a background miss, a queued settle, or a failed select, that was still the previous blank or the root — so the “new” card was in the wrong place and a later double-tap labeled the wrong node. (2) Phone CSS set `g.node * { pointer-events: none }`, so iOS delivered the tap to a reserved hit-slot *behind* the painted card. After create, slots can lag the visual; the slot under the finger was the origin/root. `editSelected` also looked up `descendants()` only, so a just-inserted `_uid` still sitting in raw data (queued settle) could not open, and the next tap resolved elsewhere.
- **Fix.** `createRelative(direction, originUid)` **requires** the finger uid — no selected/root fallback. Flick passes `candidate.uid`. Card **faces** receive pointer events (`rect:not(.grabzone)` / `text`); grabzones stay dead. Queued creates immediately `syncCreateHitSlots` so the new `_uid` has a reserved slot before the settle flush. `editSelected` resolves a missing descendant via `ensureUidLayout` (data `_uid` → layout coords + hit-slot, no name lookup). Commit still writes only `editingUid`. Empty `""` still allowed.
