...
- **Vegas Mix.** Start, the Curriculum Mix button, and M call `randomizeTree` on a slot-machine curve: beats begin close together and each gap grows, so the tumble starts fast and slows down, over about 4.6 seconds. The camera transform is restored after every beat.
...
## Labeling identity (PR 112)

- **Wrong-card editor after flick-create.** The remaining leak was not a name collision. Double-tap **re-hit-tested on pointerup**. After create, parent/root painted cards are still sliding for 260ms over the new blank’s reserved slot. `uidFromVisualPoint` preferred that mid-tween visual, so `editingUid` became the origin or root. The first tap could stamp the new `_uid` and the second tap resolve a different card, so `lastTap` never matched and a later tap “double-tapped” the root. Separately, `createRelative` once took `state.selectedUid` (stale after a background miss), and phone `g.node * { pointer-events: none }` let iOS land on a lagging hit-slot behind the face.
- **Fix.** Identity is the **touched node’s datum / `data-uid`**, captured on pointerdown (`event.__logyqUid` from the node/slot handler, then `candidate.uid`). A stationary tap / double-tap **does not re-hit** on up. Hit fallback is reserved layout slot, then point, then visual last. During `logyq-layout-settling`, painted faces are inert so reserved slots own the finger. A create that lands during an in-flight settle immediately `ensureCreateNodes` so the new `_uid` has a painted card + slot before she can tap. `createRelative(direction, originUid)` still requires the finger uid. `editSelected` still requires that uid (`ensureUidLayout` if the g.node is not joined yet). Commit writes only `editingUid`. Empty `""` still allowed.
...
- **Fix.** Double-tap calls `editSelected({ uid })` for the exact hit. Hit-test prefers the reserved layout slot / tapped `data-uid`, not mid-tween visuals.

## Create → layout → settle pipeline (PR 112)

- **Reverted `snapLaidOutNodes` on create.** `91dc667` jumped every card+link to `d.x,d.y` on flick. That stopped stacked hits but jerked the whole map (down-flick was no longer silky). The snap function is gone from create/flick and from `treeManager`.
- **One path for down/left/right/up.** `createRelative` inserts a unique `_uid` → `d3.hierarchy` + `d3.tree` once → one 260ms transition of cards and links to those finals. No `interrupt()`, no camera fly, no editor, no MIC.
- **Hits use identity, not mid-tween geometry.** Invisible `g.hit-slot[data-uid]` sit at the reserved slot immediately. `uidFromEvent` reads the tapped element’s `_uid`. Fallback `hitLayoutSlot` uses laid-out `d.x,d.y` through the zoom transform — not `getBoundingClientRect` of a card still sliding. `selectByName` is not on this path; blank labels are rejected there.
- **After settle, slots do not overlap.** Sibling/cousin spacing is the existing `nodeSize` + `separation` layout. Visual faces match those slots once the tween finishes. The crushed “Node 09 Node 22…” bottom strip in Ashley’s zoomed shot is HTML-sized dock text, not SVG cards that lost layout.
