...
Live mobile grammar: flick / 280ms hold-drag / 360ms double-tap on `LOGYQBridge`. See `ARCHITECTURE.md`.
...
- `LATCH_MAP_SHIFT` (pan map north 1.45cm on latch) plus equal card-pop via `liftPx()` / `fingerOffset` — easy to revert the map pan
...
| Mix + node context menu | **No.** Mix button and right-click dumps are live. |
| Detector overlays | **No.** Geometry is the drop engine. |
| `#showCarets` checkbox | **Only with its JS.** The box is visible; `elements.settings.showCarets` is missing so the change handler never binds. Wire the ref, or delete HTML + dead `if (s.showCarets)` together. |
| `#settingsClose` ✕ | **Only with a check.** Settings open/close currently uses backdrop click and Escape; the ✕ is not given a listener. Delete the button, or wire `s.close`. |
...
- `#showCarets` is unwired (`elements.settings` has no `showCarets`).
- `#settingsClose` has no click listener.
