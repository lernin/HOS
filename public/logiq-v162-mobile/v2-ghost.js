(() => {
  ...
  function injectStyles(doc) {
    ...
    style.id = 'logiq-v2-ghost-styles'
    style.textContent = `
  ...
  function bindGestures(doc, win, bridge, state) {
    ...
    const up = e => {
      ...
      if (g && g.pointerId === e.pointerId) {
        ...
        commitDrop(doc,win,bridge,g,e.clientX,e.clientY)
        return
      }
  ...
  function makeFloating(doc,node,rect,x,y) {
    const el = doc.createElement('div')
    el.id = 'logiq-v2-drag-card'
    el.textContent = cardText(node) || ' '
    el.style.width = `${Math.max(54,rect.width)}px`
    el.style.height = `${Math.max(34,rect.height)}px`
    doc.body.appendChild(el)
    return el
  }
  ...
  function updateFloating(win,g,x,y) {
    ...
    const left = Math.max(-width+18,Math.min(win.innerWidth-18,x-g.grabX))
  ...
  function commitDrop(doc,win,bridge,g,x,y) {
    const node = nodeByUid(doc,g.uid)
    if (!node) {
      bridge.loadMap(g.before.tree,g.before.wordBank)
      bridge.selectByUid(g.uid)
      releaseConsumedLater(win,g.pointerId)
      return
    }
    ...
    const rect = node.getBoundingClientRect()
    const sx = Math.max(rect.left+2,Math.min(rect.right-2,rect.left+Math.min(g.grabX,rect.width-2)))
    const sy = Math.max(rect.top+2,Math.min(rect.bottom-2,rect.top+Math.min(g.grabY,rect.height-2)))

    mouse(node,win,'mousedown',sx,sy,1)
    mouse(win,win,'mousemove',sx,sy,1)
    mouse(win,win,'mousemove',x,y,1)
  ...
  function releaseConsumedLater(win,pointerId) {
    win.setTimeout(() => win.__logiqV2ConsumedPointers.delete(pointerId),0)
  }
  ...
  function actionLoop(doc,win,state) {
    const tick = () => {
      ...
      if (visible && !state.editorUid && !state.gesture) {
