  ...
  return <main className="wv2-root">
    ...
    {started&&ready&&!error&&<>
      ...
      <header className="wv2-hud">
        <div><button className="wv2-pill" onClick={onBack}>← The Lab</button><div className="wv2-kicker">WATERFALL VILLAGE V2 · WALKABILITY PASS</div><h1>{current.name}</h1><p>{state.zone}</p></div>
      ...
      <aside className="wv2-legend"><strong>15 future sites</strong><span>Textured route · wider bridges · assisted approaches</span><button onClick={()=>world.current?.reset()}>Reset position</button></aside>
      <div className="wv2-bottom">
        ...
        <div className="wv2-center-note"><b>Traversal test</b><span>Try the whole loop, both bridges, and the high ground. The route should help rather than fight you.</span></div>
        <div className="wv2-look-label"><span>✧</span>LOOK</div>
      </div>
    </>}
    {(!started||!ready||error)&&<div className="wv2-overlay"><section>
      <div className="wv2-kicker">ITERATION 02 · WALKABILITY + EARLY SURFACES</div><h1>Waterfall<br/>Village V2</h1>
      <p>{error||'Same terrain ideas, rebuilt to be easier to traverse. The grass and stone are still early materials; the important test is whether walking the land now feels natural.'}</p>
      ...
      {error?<button className="wv2-primary" onClick={()=>location.reload()}>Reload workshop</button>:ready?<button className="wv2-primary" onClick={begin}>Walk {current.name} →</button>:<div className="wv2-loading">Shaping walkable terrain…</div>}
