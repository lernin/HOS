...
export function Village({ onBack }: { onBack: () => void }) {
  ...
  const input = useRef<VillageInput>({ x: 0, z: 0, yaw: -0.55, pitch: -0.04, paused: true, speed: 3.0 })
  ...
  const [state, setState] = useState<VillageState>({ position: spawn, zone: 'Village Meadow', fps: 0 })
    ...
    void createVillage(canvas.current!, input.current, controller.signal, setState, setProgress)
      .then(instance => {
        if (controller.signal.aborted) instance.dispose()
          ...
          setReady(true)
          input.current.paused = !startedRef.current
        }
      })
      .catch(err => {
        if (!controller.signal.aborted) setError(err instanceof Error ? err.message : 'Could not open the village.')
      })
    return () => {
      controller.abort()
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (k === 'escape' && event.type === 'keydown') {
        stopInput()
        input.current.paused = true
        setStarted(false)
        return
      ...
      if (event.type === 'keydown') keys.current.add(k)
  ...
  return <main className="vg-root" aria-label="Village experiment">
    ...
    {started && ready && !error && <>
      <div className="vg-look"
        onPointerDown={event => {
          if (input.current.paused || look.current) return
          event.currentTarget.setPointerCapture(event.pointerId)
          look.current = { id: event.pointerId, x: event.clientX, y: event.clientY }
        }}
        onPointerMove={event => {
          const pointer = look.current
          if (!pointer || pointer.id !== event.pointerId || input.current.paused) return
          input.current.yaw -= (event.clientX - pointer.x) * 0.0032
          input.current.pitch = Math.max(-1.0, Math.min(0.9, input.current.pitch - (event.clientY - pointer.y) * 0.0032))
          pointer.x = event.clientX
          pointer.y = event.clientY
        }}
        onPointerUp={event => { if (look.current?.id === event.pointerId) look.current = null }}
        onPointerCancel={() => { look.current = null }} />

      <header className="vg-hud">
        <button className="vg-pill" onClick={back}>← The Lab</button>
        <div className="vg-title"><span>VILLAGE · FIRST WALK</span><strong>{state.zone}</strong></div>
        <button className="vg-pill" onClick={reset}>Reset</button>
      </header>

      <div className="vg-bottom">
        <div className="vg-stick-wrap">
          <div className="vg-stick"
            onPointerDown={event => {
            ...
            onPointerMove={event => {
              const start = stick.current
              if (!start || start.id !== event.pointerId || input.current.paused) return
              const dx = event.clientX - start.x
              const dz = event.clientY - start.y
              const length = Math.hypot(dx, dz)
              const distance = Math.min(46, length)
              const gain = length > 6 ? (distance - 6) / 40 / (length || 1) : 0
              input.current.x = dx * gain
              input.current.z = dz * gain
              if (nub.current) nub.current.style.transform = `translate(${dx / (length || 1) * distance}px,${dz / (length || 1) * distance}px)`
            }}
            onPointerUp={releaseStick}
            onPointerCancel={releaseStick}>
            <span className="vg-ring" />
            <span ref={nub} className="vg-nub" />
          </div>
          <span>WALK</span>
        </div>
        <div className="vg-note"><strong>Explore first.</strong><span>Cross the creek. Find the open cottage door. Walk inside.</span></div>
    ...
    {(!started || !ready || error) && <div className="vg-overlay"><section>
      <div className="vg-kicker">THE LAB · ENVIRONMENT PROTOTYPE 01</div>
      <h1>Creekside<br />Village</h1>
      <p>{error || progress}</p>
      {!error && <p className="vg-subcopy">A small valley, running water, a wooden bridge and three Quaternius buildings. One cottage is genuinely enterable.</p>}
      {error
        ? <button className="vg-primary" onClick={() => location.reload()}>Reload village</button>
        : ready
          ? <button className="vg-primary" onClick={begin}>Walk into the village →</button>
          : <div className="vg-loading">{progress}</div>}
