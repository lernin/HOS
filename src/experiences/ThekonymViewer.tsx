...
import { confidenceNames, examples, fluencyNames, freshCopy, maturityNames, paperUrl, priorityExplanation, pronunciation, score, searchCatalogue, writeClipboard } from '../lib/thekonymViewer'
...
function Section({ title, confidence, children, className = '' }: { title: string; confidence?: Confidence; children: ReactNode; className?: string }) {
  return <section className={`tv-section ${className}`}><h2>{title}{confidence !== undefined && <ConfidenceMark value={confidence} />}</h2>{children}</section>
}
...
export function ThekonymViewer({ source, onExit, initialData, onSignOut }: { source: ViewerSource; onExit: () => void; initialData?: ViewerInitialData; onSignOut?: () => void }) {
  ...
  const freshLabel = source.mode === 'snapshot' ? `Preview snapshot · ${dateLabel(source.capturedAt)}` : busy ? 'Checking production…' : !online ? `Offline · last checked ${dateLabel(checkedAt)}` : error ? `Refresh failed · ${checkedAt ? `last checked ${dateLabel(checkedAt)}` : 'not connected'}` : `Production · checked ${dateLabel(checkedAt)}`
    ...
    <div className="tv-layout">
      ...
      <div className="tv-reading" ref={scroller}>
        ...
        {current && <article className="tv-page" aria-busy={busy}>
          ...
          <Section title="Examples" confidence={current.example_confidence}>{examples(current.example).length ? <ol className="tv-examples">{examples(current.example).map((e, i) => <li key={i}><span>{String(i + 1).padStart(2, '0')}</span><p>{e}</p></li>)}</ol> : <p><Missing /></p>}</Section>
          ...
          <Section title="Priority"><dl className="tv-facts"><Fact label="Calculated priority">{current.priority == null ? <Missing text="Awaiting inputs" /> : <strong>{Number(current.priority.toFixed(2))}</strong>}</Fact><Fact label="Target phase"><Value>{current.target_phase}</Value></Fact><Fact label="Application priority"><Value>{current.application_priority}</Value></Fact></dl><div className="tv-formula">{priorityExplanation(current)}</div><details className="tv-details"><summary>How priority is calculated</summary><p>((table weight × field weight) + application priority) ÷ target phase.</p><p>Table weight: 5 if a table, otherwise 1. Field weight: 2 if “Field of” has a recorded entry, otherwise 1. Missing required inputs leave priority uncalculated. Confidence does not enter this formula.</p></details></Section>
