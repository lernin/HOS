...
export function OceanEstate({onBack}:{onBack:()=>void}){
  ...
  return <main className="oe-root" aria-label="Ocean Estate" data-player-x={state.position.x.toFixed(3)} data-player-z={state.position.z.toFixed(3)} data-camera-yaw={input.current.yaw.toFixed(3)}>
    <canvas ref={canvas} className="oe-canvas" aria-label="Explorable oceanfront estate" onContextMenu={e=>e.preventDefault()}
      onPointerDown={e=>{if(input.current.paused||pointer.current||artOpen.current)return;e.currentTarget.setPointerCapture(e.pointerId);pointer.current={id:e.pointerId,x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY,dragged:false};if(prefs.mode==='walk'&&e.clientY>innerHeight*.58)input.current.z=-1}}
      ...
      onPointerUp={e=>{const p=pointer.current;if(!p||p.id!==e.pointerId)return;if(!p.dragged&&prefs.mode==='explore'&&!input.current.paused){const picked=engine.current?.pick(e.clientX,e.clientY);if(!picked)setNotice('Tap a clear spot on the floor or a painting.');else if(picked.kind==='art')enterArt(picked.art);else setHelp(false)}pointer.current=null;input.current.z=0}}
    ...
    {started&&!error&&!art&&<>
      ...
      {help&&!places&&!state.moving&&<div className="oe-hint">{prefs.mode==='explore'?'Tap the floor to walk · Tap art to examine · Drag to look':'Hold low on the screen to walk · Drag to steer'}<small>Keyboard: WASD · Shift to walk faster</small><button onClick={()=>setHelp(false)} aria-label="Dismiss controls hint">×</button></div>}
    </>}
