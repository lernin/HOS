...
export function OceanEstate({onBack}:{onBack:()=>void}){
  ...
  useEffect(()=>{if(!editMode||!activeRoom||!started||paused||picker){setMarkers({floor:{x:0,y:0,visible:false},walls:{x:0,y:0,visible:false}});return}
  ...
  },[editMode,activeRoom,started,paused,picker])
  ...
  function closeSettings(){stop();if(started&&!paused&&!picker){input.current.paused=false;if(sound)void beginSound()}}
  function back(){stop();input.current.paused=true;audio.current?.pause();onBack()}
  function openPicker(surface:EditableSurface){if(!activeRoom)return;stop();input.current.paused=true;setPicker({room:activeRoom,surface,original:selections[activeRoom][surface],current:selections[activeRoom][surface]})}
  ...
  function finishPicker(keep:boolean){if(!picker)return;const value=keep?picker.current:picker.original;engine.current?.setRoomMaterial(picker.room,picker.surface,value);if(keep){const next={...selections,[picker.room]:{...selections[picker.room],[picker.surface]:value}} as RoomSelections;setSelections(next);try{localStorage.setItem('ocean-estate-room-materials-v1',JSON.stringify(next))}catch{/* Keep in memory. */};setNotice(`${roomName(picker.room)} ${picker.surface==='floor'?'floor':'walls'} saved.`)}setPicker(null);if(started&&!paused)input.current.paused=false}
  return <main className="oe-root" aria-label="Ocean Estate" data-player-x={state.position.x.toFixed(3)} data-player-z={state.position.z.toFixed(3)} data-camera-yaw={input.current.yaw.toFixed(3)}>
    ...
    {started&&!error&&<>
      ...
      {editMode&&<div className="oe-edit-badge"><span>EDIT MODE</span>{activeRoom?<strong>{roomName(activeRoom)}</strong>:<strong>Foyer + Great room enabled</strong>}</div>}
      {editMode&&activeRoom&&!picker&&(['floor','walls'] as EditableSurface[]).map(surface=>{const p=markers[surface];return p.visible?<button key={surface} className={`oe-surface-marker oe-surface-${surface}`} style={{left:p.x,top:p.y}} onClick={()=>openPicker(surface)} aria-label={`Edit ${surface}`}><span>⚙</span><strong>{surface==='floor'?'Floor':'Walls'}</strong></button>:null})}
      ...
      {help&&!places&&!state.moving&&!editMode&&<div className="oe-hint">{prefs.mode==='explore'?'Tap the floor to walk · Drag to look':'Hold low on the screen to walk · Drag to steer'}<small>Keyboard: WASD · Shift to walk faster</small><button onClick={()=>setHelp(false)} aria-label="Dismiss controls hint">×</button></div>}
    </>}
    ...
    <dialog ref={dialog} className="oe-settings" onClose={closeSettings}><form method="dialog"><header><span>YOUR STAY</span><button aria-label="Close settings">×</button></header></form><h2>A little more comfortable.</h2>
      <label>Design editing<select aria-label="Design editing" value={editMode?'on':'off'} onChange={e=>{setEditMode(e.target.value==='on');setHelp(false)}}><option value="off">Off · just explore</option><option value="on">On · edit room surfaces</option></select></label>
