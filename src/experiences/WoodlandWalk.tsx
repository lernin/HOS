...
export function WoodlandWalk({onBack}:{onBack:()=>void}){
  ...
  function auditionMusic(id:string){try{void ensureAudio().audition(id).catch(()=>setSoundError(true))}catch{setSoundError(true)}}
  return <main className="woodland">
    ...
    <dialog ref={dialog} className="woodland-dialog" onClose={()=>{stop();input.current.paused=!started}}>
      ...
      <WoodlandMusicPalette ratings={musicRatings} onRate={rateMusic} onAudition={auditionMusic}/>
      <label>Look sensitivity <input type="range" min=".4" max="2" step=".1" value={sensitivity} onChange={e=>setSensitivity(Number(e.target.value))}/></label>
      ...
      <button onClick={()=>{world.current?.reset();dialog.current?.close()}}>Return to the trail entrance</button><button onClick={()=>dialog.current?.close()}>Done</button>
    </dialog>
  </main>
}
