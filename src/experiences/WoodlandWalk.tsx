...
export function WoodlandWalk({onBack}:{onBack:()=>void}){
  ...
  const [lookGain,setLookGain]=useState(1),[cameraMass,setCameraMass]=useState(.28),[flickGlide,setFlickGlide]=useState(.32)
    ...
    const tick=(now:number)=>{
      ...
      if(!input.current.paused){
        if(!look.current){
          ...
          const drag=3.2-2.85*flickGlide
          const decay=Math.exp(-drag*dt)
          flickVelocity.current.yaw*=decay;flickVelocity.current.pitch*=decay
          if(Math.abs(flickVelocity.current.yaw)<.003)flickVelocity.current.yaw=0
          if(Math.abs(flickVelocity.current.pitch)<.003)flickVelocity.current.pitch=0
        }
  ...
  function endLook(){
    ...
    if(p){flickVelocity.current.yaw=p.vyaw;flickVelocity.current.pitch=p.vpitch}
    look.current=null
  }
  return <main className="woodland">
    ...
    <div className="woodland-look" aria-label="Drag to look around" onPointerDown={e=>{if(input.current.paused||look.current)return;e.currentTarget.setPointerCapture(e.pointerId);flickVelocity.current={yaw:0,pitch:0};look.current={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now(),vyaw:0,vpitch:0}}} onPointerMove={e=>{const p=look.current;if(!p||p.id!==e.pointerId||input.current.paused)return;const now=performance.now(),dt=Math.max(.008,(now-p.t)/1000),dx=e.clientX-p.x,dy=e.clientY-p.y,yawDelta=-dx*.003*lookGain,pitchMove=-dy*.003*lookGain;targetLook.current.yaw+=yawDelta;const before=targetLook.current.pitch;targetLook.current.pitch=pitchDelta(before,pitchMove);const appliedPitch=targetLook.current.pitch-before;p.vyaw=p.vyaw*.45+(yawDelta/dt)*.55;p.vpitch=p.vpitch*.45+(appliedPitch/dt)*.55;p.x=e.clientX;p.y=e.clientY;p.t=now}} onPointerUp={endLook} onPointerCancel={()=>{flickVelocity.current={yaw:0,pitch:0};look.current=null}}/>
    ...
    <dialog ref={dialog} className="woodland-dialog" onClose={closeSettings}>
      ...
      <section className="woodland-camera">
        ...
        <label>Flick glide <strong>{Math.round(flickGlide*100)}</strong><input type="range" min="0" max="1" step=".01" value={flickGlide} onChange={e=>setFlickGlide(Number(e.target.value))}/><small>Higher = a quick flick carries farther before friction stops it.</small></label>
      </section>
