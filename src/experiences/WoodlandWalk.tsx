...
export function WoodlandWalk({onBack}:{onBack:()=>void}){
  ...
  const [lookGain,setLookGain]=useState(1),[cameraMass,setCameraMass]=useState(.28),[flickGlide,setFlickGlide]=useState(3),[moveAcceleration,setMoveAcceleration]=useState(2),[viewMode,setViewMode]=useState(0),[hybridControls,setHybridControls]=useState(false),[showMap,setShowMap]=useState(false)
  ...
  const releaseMove=()=>{clearMove();if(hybridControls&&look.current){look.current.ax=look.current.x;look.current.ay=look.current.y;look.current.vyaw=0;look.current.vpitch=0;clearWander()}}
