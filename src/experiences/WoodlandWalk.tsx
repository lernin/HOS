...
export function WoodlandWalk({onBack}:{onBack:()=>void}){
  ...
  function ensureAudio(){
    ...
    if(!score.current){
      score.current=woodlandWorldScore()
      score.current.setVolume(music)
      score.current.setMood(currentMoodRef.current)
    }
  ...
  function begin(){
    ...
    setStarted(true)
    try{
      const players=ensureAudio()
      void Promise.all([players.naturePlayer.start(),players.scorePlayer.start()]).catch(()=>setSoundError(true))
    }catch{setSoundError(true)}
  }
  function settings(){
    stop();input.current.paused=true
    audio.current?.pauseAmbient();score.current?.pause()
  ...
  return <main className="woodland">
    ...
    <dialog ref={dialog} className="woodland-dialog" onClose={closeSettings}>
      ...
      <section className="woodland-music"><div className="woodland-music-heading"><div><h3>Musical world</h3><p>Current color: <b>{currentMoodInfo.emoji} {currentMoodInfo.label}</b>. Temporary signposts around the main loop trigger the ten emotional colors. Melody Lab ratings choose the preferred theme for each color.</p></div><span>V0</span></div></section>
