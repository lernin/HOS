...
export function createEstateKit(scene:T.Scene){
  ...
  function material(name:string){if(mats.has(name))return mats.get(name)!
    const metal=['bronze','gold','black'].includes(name),fabric=['linen','sage','clay','indigo','rug'].includes(name),wood=['oak','walnut','bark'].includes(name)
    const m=new T.MeshStandardMaterial({color:colors[name]||name,roughness:metal?.28:fabric?.94:wood?.63:.74,metalness:metal?.8:0})
