...
import { floors, walls, glass, FLOOR } from './plan'
...
export function architecture(k:EstateKit){
  for(const f of floors){const w=f.x2-f.x1,d=f.z2-f.z1,x=(f.x1+f.x2)/2,z=(f.z1+f.z2)/2,y=f.level??FLOOR
    ...
    k.box(x,y-.2,z,w,.4,d,f.material)
    ...
    if(f.roof){k.box(x,y+f.roof+.22,z,w+.65,.4,d+.65,'travertine');k.box(x,y+f.roof-.02,z,w,.035,d,'plaster')
  ...
  for(const x of [-1.3,3.3]){const g=k.group(x,FLOOR,24,x<0?-.98:.98);k.box(x<0?-1.04:1.04,1.8,0,2.04,3.6,.13,'walnut',g,.035);k.box(x<0?-1.8:1.8,1.7,-.1,.03,1.1,.04,'bronze',g)}
...
export function landscape(k:EstateKit){
  ...
  for(let j=0;j<=rings;j++)for(let i=0;i<=segments;i++){const a=i/segments*Math.PI*2,r=j/rings,edge=1+.04*Math.sin(a*7)+.025*Math.sin(a*13),x=Math.cos(a)*58*r*edge,z=7+Math.sin(a)*64*r*edge;verts.push(x,terrainHeight(x,z,r),z)}
  for(let j=0;j<rings;j++)for(let i=0;i<segments;i++){const a=j*(segments+1)+i,c=a+segments+1;ids.push(a,c,a+1,a+1,c,c+1)}
  ...
  for(let i=0;i<150;i++){const a=i/150*Math.PI*2,r=.9+rand()*.09,x=Math.cos(a)*58*r,z=7+Math.sin(a)*64*r;rock(x,-.2+rand()*1.2,z,2+rand()*3,1.5+rand()*4,2+rand()*3,i+24)}
  ...
  function frond(g:T.Group,angle:number,length:number){const points=[v(0,0,0),v(Math.cos(angle)*length*.4,length*.28,Math.sin(angle)*length*.4),v(Math.cos(angle)*length,length*.03,Math.sin(angle)*length)];k.beam(points,.027,'leafLight',g,5)
    for(let i=1;i<=10;i++){const t=i/11,cx=Math.cos(angle)*length*t,cz=Math.sin(angle)*length*t,cy=Math.sin(t*Math.PI)*length*.25;for(const s of [-1,1]){
      const l=length*.25*Math.sin(t*Math.PI),dx=Math.cos(angle+s*.85)*l,dz=Math.sin(angle+s*.85)*l
      const p=[cx,cy,cz,cx+dx*.5,cy+.08,cz+dz*.5,cx+dx,cy-.2*t,cz+dz,cx+dx*.48+Math.sin(angle)*.1,cy-.035,cz+dz*.48-Math.cos(angle)*.1]
  ...
  function tree(x:number,y:number,z:number,size:number,seed:number){const g=k.group(x,y,z,seed);g.scale.setScalar(size)
    ...
    for(let i=0;i<5;i++){const a=i*2.4+seed,dx=Math.cos(a)*1.7,dz=Math.sin(a)*1.7;k.beam([v(0,1.6+i*.38,.1),v(dx*.5,3+i*.25,dz*.5),v(dx,4+i*.23,dz)],.08,'bark',g)
      for(let j=0;j<4;j++){const m=k.mesh(new T.IcosahedronGeometry(1,1),j%3===0?'leafLight':j%3===1?'leaf':'leafDark',dx+Math.cos(j*2)*.6,4+i*.23+Math.sin(j)*.24,dz+Math.sin(j*2)*.6,g);m.scale.set(1.1,.62,.95);m.rotation.y=seed+j}
  ...
  for(let i=0;i<42;i++){const a=i/42*Math.PI*2,r=.75+rand()*.06,x=Math.cos(a)*58*r,z=7+Math.sin(a)*64*r;if(x>42&&z<-12)continue;if(i%4===0)tree(x,terrainHeight(x,z,r),z,.75+rand()*.45,i);else palm(x,terrainHeight(x,z,r),z,3.5+rand()*2,i)}
  for(let i=0;i<210;i++){const a=rand()*Math.PI*2,r=.7+rand()*.18,x=Math.cos(a)*58*r,z=7+Math.sin(a)*64*r;grasses(x,terrainHeight(x,z,r)+.05,z,1+rand());if(i%6===0)rock(x,terrainHeight(x,z,r),z,.7,.5,.65,i)}
...
export function waters(scene:T.Scene){
  ...
  const water=new T.ShaderMaterial({uniforms:{...common,pool:{value:0}},side:T.DoubleSide,vertexShader:`varying vec3 wp;uniform float time;uniform float pool;void main(){vec3 p=position;if(pool<.5)p.z+=sin(p.x*.055+time*.35)*.24+sin(p.y*.083-time*.3)*.19;vec4 w=modelMatrix*vec4(p,1.);wp=w.xyz;gl_Position=projectionMatrix*viewMatrix*w;}`,fragmentShader:`
    ...
    void main(){vec2 p=wp.xz;float a=sin(p.x*.65+time*.65+sin(p.y*.5))*sin(p.y*.75-time*.45);float b=sin(p.x*3.3+p.y*1.4-time)*sin(p.y*2.7+time*.7);vec3 n=normalize(vec3(a*.09+b*.025,1.,cos(p.y*.7-time*.6)*.1));vec3 view=normalize(cameraPosition-wp);float fres=pow(1.-max(0.,dot(n,view)),4.);vec3 deep=mix(vec3(.035,.24,.28),vec3(.10,.48,.47),pool);vec3 sky=mix(vec3(.67,.79,.79),vec3(.15,.24,.32),evening);vec3 col=mix(deep,sky,fres*.8);float spec=pow(max(0.,dot(reflect(-normalize(vec3(-.5,.24,-.8)),n),view)),140.);col+=vec3(1.,.79,.45)*spec*.55*(1.-evening*.8);float caustic=pow(max(0.,sin(p.x*2.5+sin(p.y*3.+time*.4))+sin(p.y*2.7+time*.25)-.9),3.);col+=pool*caustic*.045;col+=a*.018+b*.008;float shore=(1.-smoothstep(54.,62.,length(vec2(p.x,(p.y-7.)*.91))))*smoothstep(48.,55.,length(vec2(p.x,(p.y-7.)*.91)));col+=shore*(.1+.06*sin(length(p)*3.-time));float haze=1.-exp(-distance(cameraPosition,wp)*.0016);col=mix(col,sky,haze);col*=1.-evening*.48;gl_FragColor=vec4(col,1.);#include <tonemapping_fragment>\n#include <colorspace_fragment>}
  `.replace(';#include',';\n#include')})
