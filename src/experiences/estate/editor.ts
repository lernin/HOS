...
export const estateEditorMaterials:EditorMaterial[]=[
  {id:'marble01',label:'Cream Marble 01',slug:'marble_01',surface:'floor',use:'Calm cream stone · subtly varied',meters:1.5},
...
export function createEstateEditor(scene:T.Scene,renderer:T.WebGLRenderer,camera:T.Camera,canvas:HTMLCanvasElement){
  ...
  function materialFor(id:string,repeatX:number,repeatY:number){
    ...
    if(def.surface==='floor'&&def.id==='marble01')return permutedStoneMaterial(def)
  ...
  function setMaterial(room:EditableRoomId,surface:EditableSurface,id:string|null){
    for(const mesh of surfaces[room][surface]){
      ...
      if(id){mesh.geometry.computeBoundingBox();const size=mesh.geometry.boundingBox!,w=Math.max(.1,size.max.x-size.min.x,size.max.z-size.min.z),h=Math.max(.1,size.max.y-size.min.y,size.max.z-size.min.z);const next=materialFor(id,w,h);if(next){mesh.material=next;mesh.visible=true}}
  ...
  return {setMaterial,select,pick,project,dispose(){for(const room of ['foyer','great-room'] as EditableRoomId[])for(const surface of ['floor','walls'] as EditableSurface[]){for(const m of surfaces[room][surface])scene.remove(m);for(const m of targets[room][surface])scene.remove(m)}textures.forEach(t=>t.dispose());materials.forEach(m=>{if(m instanceof T.MeshStandardMaterial){m.map?.dispose();m.roughnessMap?.dispose();m.normalMap?.dispose()}m.dispose()});geometries.forEach(g=>g.dispose())}}
}
