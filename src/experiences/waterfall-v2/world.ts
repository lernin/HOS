...
export type RouteProjection = { x: number; z: number; distance: number; segment: number; t: number }
...
function terrainWithPads(id: ConceptId, x: number, z: number) {
...
function makeBridge(id: ConceptId, z: number, primary: boolean): BridgeSpec {
  const centerX = riverCenter(id,z), width = primary ? 5.8 : 4.0, halfLength = primary ? 13.5 : 10.5
  const left = terrainWithPads(id,centerX-halfLength,z), right = terrainWithPads(id,centerX+halfLength,z)
  return { name: primary ? 'Grand Arch' : 'Lower Crossing', z, centerX, halfLength, width, baseY: Math.max(left,right)+.18, archHeight: primary ? 3.0 : .65, primary }
}
...
export function bridgeAt(id: ConceptId, x: number, z: number) { return getBridges(id).find(b => Math.abs(z-b.z) <= b.width*.52 && Math.abs(x-b.centerX) <= b.halfLength+.45) }
...
export function projectToRoute(id: ConceptId, x: number, z: number): RouteProjection {
  const loop=mainLoop(id);let best:RouteProjection={x:loop[0].x,z:loop[0].z,distance:Infinity,segment:0,t:0}
  for(let i=0;i<loop.length-1;i++){
    const a=loop[i],b=loop[i+1],dx=b.x-a.x,dz=b.z-a.z,len2=dx*dx+dz*dz||1
    const t=Math.max(0,Math.min(1,((x-a.x)*dx+(z-a.z)*dz)/len2)),px=a.x+dx*t,pz=a.z+dz*t,distance=Math.hypot(x-px,z-pz)
    if(distance<best.distance)best={x:px,z:pz,distance,segment:i,t}
  }
  return best
}

function routeBaseHeight(id:ConceptId,x:number,z:number){const bridge=bridgeAt(id,x,z);return bridge?bridgeHeight(bridge,x):terrainWithPads(id,x,z)}

export function terrainHeight(id: ConceptId, x: number, z: number) {
  let y=terrainWithPads(id,x,z)
  const route=projectToRoute(id,x,z),loop=mainLoop(id)
  if(route.distance<6.4){
    const a=loop[route.segment],b=loop[Math.min(loop.length-1,route.segment+1)]
    const target=routeBaseHeight(id,a.x,a.z)+(routeBaseHeight(id,b.x,b.z)-routeBaseHeight(id,a.x,a.z))*route.t
    const influence=1-smooth(2.1,6.4,route.distance)
    y+=(target-y)*influence*.9
  }
  return y
}
...
export function canStand(id: ConceptId, x: number, z: number) { return Math.abs(x) < WORLD_HALF-1.5 && Math.abs(z) < WORLD_HALF-1.5 && (!isRiver(id,x,z,.05) || Boolean(bridgeAt(id,x,z))) }

export function assistedPoint(id:ConceptId,x:number,z:number,strength=.18){
  const bridge=getBridges(id).find(b=>Math.abs(x-b.centerX)<=b.halfLength+5&&Math.abs(z-b.z)<=b.width*1.25+2)
  if(bridge)return {x,z:z+(bridge.z-z)*Math.max(strength,.28)}
  const route=projectToRoute(id,x,z)
  if(route.distance<4.8)return {x:x+(route.x-x)*strength,z:z+(route.z-z)*strength}
  return {x,z}
}
