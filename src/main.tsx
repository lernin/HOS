...
type View = 'hub' | 'thekonym-viewer' | 'thekonym' | 'world3d' | 'water-garden' | 'woodland-walk' | 'backing-tracks' | 'orchestration-lab' | 'melody-lab' | 'roy' | 'ekpronym' | 'library' | 'scroller' | 'bookvocab'
...
const MelodyLab = lazy(() => import('./experiences/MelodyLab').then(module => ({ default: module.MelodyLab })))
...
function App() {
  const [view, setView] = useState<View>(() => window.location.pathname === '/woodland-walk' ? 'woodland-walk' : window.location.pathname === '/backing-tracks' ? 'backing-tracks' : window.location.pathname === '/orchestration-lab' ? 'orchestration-lab' : window.location.pathname === '/melody-lab' ? 'melody-lab' : window.location.pathname === '/water-garden' ? 'water-garden' : window.location.pathname === '/thekonym-viewer' ? 'thekonym-viewer' : 'hub')
  ...
  useEffect(() => {
    if (!['/thekonym-viewer', '/water-garden', '/woodland-walk', '/backing-tracks', '/orchestration-lab', '/melody-lab'].includes(window.location.pathname)) window.history.replaceState({}, '', '/')
    const syncView = () => setView(window.location.pathname === '/woodland-walk' ? 'woodland-walk' : window.location.pathname === '/backing-tracks' ? 'backing-tracks' : window.location.pathname === '/orchestration-lab' ? 'orchestration-lab' : window.location.pathname === '/melody-lab' ? 'melody-lab' : window.location.pathname === '/water-garden' ? 'water-garden' : window.location.pathname === '/thekonym-viewer' ? 'thekonym-viewer' : window.location.pathname === '/thekonym' ? 'thekonym' : window.location.pathname === '/world-3d' ? 'world3d' : window.location.pathname === '/roy' ? 'roy' : window.location.pathname === '/ekpronym' ? 'ekpronym' : window.location.pathname === '/library' ? 'library' : window.location.pathname === '/scroller' ? 'scroller' : window.location.pathname === '/book-vocab' ? 'bookvocab' : 'hub')
    window.addEventListener('popstate', syncView)
    return () => window.removeEventListener('popstate', syncView)
  }, [])

  function navigate(next: View) {
    const path = next === 'woodland-walk' ? '/woodland-walk' : next === 'backing-tracks' ? '/backing-tracks' : next === 'orchestration-lab' ? '/orchestration-lab' : next === 'melody-lab' ? '/melody-lab' : next === 'water-garden' ? '/water-garden' : next === 'thekonym-viewer' ? '/thekonym-viewer' : next === 'thekonym' ? '/thekonym' : next === 'world3d' ? '/world-3d' : next === 'roy' ? '/roy' : next === 'ekpronym' ? '/ekpronym' : next === 'library' ? '/library' : next === 'scroller' ? '/scroller' : next === 'bookvocab' ? '/book-vocab' : '/'
  ...
  if (view === 'hub') return (
    <main className="shell hub-shell" data-theme={theme} style={styleVars}>
      ...
      <section className="experience-grid">
        ...
        <article className="experience-card">
          <span className="experience-icon">Me</span>
          <span className="experience-copy"><strong>Melody Lab</strong><small>Hear theme candidates for Hearth, Wonder, Adventure, Mystery, Triumph and the other musical-world colors with full orchestral accompaniment. Rank what you love.</small></span>
          <button className="experience-go" onClick={() => navigate('melody-lab')}>Go</button>
        </article>
        <article className="experience-card">
  ...
  if (view === 'melody-lab') return <Suspense fallback={<main className="shell"><div className="center">Opening Melody Lab…</div></main>}><MelodyLab onExit={() => navigate('hub')} /></Suspense>
