...
test('engine exposes a shared logyq API bag that fragments register onto', () => {
  ...
  assert.match(treeManager, /const widthScale = \(fullW - pad\) \/ b\.width/)
  ...
  assert.match(bridge, /immediate: true/)
  assert.match(bridge, /snapLaidOutNodes/)
  ...
  assert.match(treeManager, /snapLaidOutNodes\(\)/)
  assert.match(treeManager, /const immediate = !!\(opts && opts\.immediate\)/)
  assert.match(treeOps, /immediate: !!opts\.immediate/)
...
test('preview fragments concatenate to the served enhancement without edits', () => {
  ...
  assert.match(assembled.source, /snapLaidOutNodes/)
