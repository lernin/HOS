...
test('LOGYQ preview uses live logiq_maps PIN RPCs; engine does not', () => {
  ...
  assert.match(preview, /\/api\/logyq-maps/)
  assert.doesNotMatch(preview, /Enter the Lab PIN to open live maps/)
  const mapsApi = readFileSync(join(root, 'api/logyq-maps.ts'), 'utf8')
  assert.match(mapsApi, /logiq_map_list/)
  assert.match(mapsApi, /logiq_map_save/)
  assert.match(mapsApi, /logiq_map_delete/)
  const pin = mapsApi.match(/ACCESS_PIN = '([^']+)'/)?.[1]
  assert.ok(pin)
  assert.doesNotMatch(preview, new RegExp(pin))
