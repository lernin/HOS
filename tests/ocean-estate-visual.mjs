...
try{
 ...
 touch.on('pageerror',e=>errors.push(e.message));await touch.goto('http://127.0.0.1:4173/tests/ocean-estate-preview.html',{waitUntil:'load'});await touch.getByRole('button',{name:'Step inside'}).waitFor({state:'visible',timeout:120000});await touch.getByRole('button',{name:'Step inside'}).tap()
 ...
 await writeFile(`${output}/verification.json`,JSON.stringify({errors,diagnostics,phoneViewport:{width:412,height:915},renderer:'Chromium SwiftShader; not S23 hardware',ui:'entry, settings, lighting, destinations, navigation start/stop, emulated touch tap-to-walk and drag-to-look, modal movement pause',routeTests:'see Node test output'},null,2))
