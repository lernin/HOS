...
try{
 ...
 await page.getByRole('button',{name:'02 Great room'}).click();await page.getByRole('button',{name:'Stop · Great room'}).waitFor({state:'visible',timeout:10000});await page.getByRole('button',{name:'Stop · Great room'}).click()
...
}finally{await writeFile(`${output}/console-errors.json`,JSON.stringify(errors,null,2));await browser.close()}
