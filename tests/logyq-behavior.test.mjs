...
test('preview gestures expose v162 flick/hold/double-tap seams and have no spawn puck', () => {
  ...
  assert.match(v162, /OFFSET_UP_CM: 1\.45/)
  assert.match(v162, /OFFSET_SIDE_CM: 0/)
  assert.match(v162, /LATCH_MAP_SHIFT: true/)
  assert.match(v162, /function liftPx/)
  assert.match(v162, /latchShiftPx\(\) \{\s*return liftPx\(\)/)
  assert.match(v162, /y: -D/)
  assert.doesNotMatch(v162, /if \(C\.LATCH_MAP_SHIFT\) return \{ x: 0, y: 0 \}/)
  ...
  assert.match(v162, /function shiftMapOnLatch/)
