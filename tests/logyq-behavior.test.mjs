...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  assert.equal(smite.smiteLinePhase(1, ...box), 'veil')
  assert.equal(smite.smiteLinePhase(smite.smiteRingFraction(15000), ...box), 'veil')
  assert.equal(smite.smiteLinePhase(0.9, ...box), 'veil')
  ...
  const veil = smite.smiteHeat('veil', 'red')
  const light = smite.smiteHeat('light', 'red')
  const medium = smite.smiteHeat('medium', 'red')
  const dark = smite.smiteHeat('dark', 'red')
  const glow = smite.smiteHeat('glow', 'red')
  assert.equal(veil.stroke, '#fecaca')
  assert.equal(light.stroke, '#fca5a5')
  assert.equal(medium.stroke, '#ef4444')
  assert.equal(dark.stroke, '#b91c1c')
  assert.equal(dark.glow, 0)
  assert.equal(glow.stroke, '#ff2d2d')
  assert.ok(glow.glow > 0.5)
  assert.ok(veil.washOpacity < light.washOpacity)
  assert.ok(light.washOpacity < medium.washOpacity)
  assert.ok(medium.washOpacity < dark.washOpacity)
  assert.ok(dark.washOpacity < glow.washOpacity)
  assert.ok(veil.washOpacity >= 0.2 && veil.washOpacity < 0.3)
  assert.equal(smite.smiteHeat('veil', 'amber').stroke, '#fde68a')
  assert.equal(smite.smiteHeat('glow', 'amber').stroke, '#ffb000')
  assert.notEqual(smite.smiteHeat('glow', 'amber').wash, glow.wash)
