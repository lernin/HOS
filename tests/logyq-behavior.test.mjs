...
test('inline edit goes through openNodeEditor, not a second helper', () => {
  ...
  assert.match(editing, /logyq\.camera\.flyEditFocusToUID/)
...
function loadSmitePure() {
  ...
  assert.ok(start >= 0 && end > start)
  return new Function(`${source.slice(start, end)}; return { smiteZone, smiteCastDirection, smiteAffected, smiteNextMark, smiteRingFraction, smiteRefillMs, planSmiteCommit, smiteClockSegments, smiteSegmentsLit, smiteHeat, smiteScarOpacity, smiteScarBlocked };`)()
}
...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  assert.equal(smite.smiteSegmentsLit(1), 12)
  assert.equal(smite.smiteSegmentsLit(smite.smiteRingFraction(15000)), 12)
  assert.equal(smite.smiteSegmentsLit(smite.smiteRingFraction(12000)), 12)
  assert.equal(smite.smiteSegmentsLit(smite.smiteRingFraction(11000)), 11)
  assert.equal(smite.smiteSegmentsLit(smite.smiteRingFraction(10000)), 10)
  assert.equal(smite.smiteSegmentsLit(0.5), 6)
  assert.equal(smite.smiteSegmentsLit(smite.smiteRingFraction(1000)), 1)
  assert.equal(smite.smiteSegmentsLit(0), 0)

  const segs = smite.smiteClockSegments(0, 0, 140, 63, 10, 10)
  assert.equal(segs.length, 12)
  assert.ok(segs.every((d) => /^M [\d.-]+ [\d.-]+ L [\d.-]+ [\d.-]+$/.test(d)), 'ticks are straight edges, not corner arcs')
  assert.equal(segs[0], 'M 72 0 L 99 0', 'first click is the top tick just right of 12')
  assert.equal(segs[1], 'M 103 0 L 130 0')
  assert.equal(segs[2].startsWith('M 140 '), true, 'then the right side, top to bottom')
  assert.ok(segs[4].endsWith(' 63'), 'then the bottom, from the right')
  assert.ok(segs[8].startsWith('M 0 '), 'then the left side')
  assert.equal(segs[11], 'M 41 0 L 68 0', 'last ticks are the top, just left of 12')
  assert.equal(smite.smiteClockSegments(0, 0, 0, 40).length, 0)
  assert.deepEqual(smite.smiteClockSegments(0, 0, 140, 63, 0, 0), segs)
  assert.equal(segs.filter((d) => d.endsWith(' 0')).length, 4, 'four ticks live on the top edge')
  assert.equal(segs.filter((d) => d.includes(' 63')).length, 4, 'four ticks live on the bottom edge')

  assert.equal(smite.smiteHeat(12, 'red').fill, '#ffffff')
  assert.equal(smite.smiteHeat(12, 'red').text, '#1f2937')
  assert.equal(smite.smiteHeat(12, 'red').stroke, '#dc2626')
  assert.equal(smite.smiteHeat(10, 'red').fill, '#fecaca')
  assert.equal(smite.smiteHeat(8, 'red').fill, '#fca5a5')
  assert.equal(smite.smiteHeat(7, 'red').text, '#1f2937')
  assert.notEqual(smite.smiteHeat(7, 'red').fill, smite.smiteHeat(6, 'red').fill)
  assert.notEqual(smite.smiteHeat(6, 'red').fill, smite.smiteHeat(5, 'red').fill)
  assert.notEqual(smite.smiteHeat(5, 'red').fill, smite.smiteHeat(4, 'red').fill)
  assert.equal(smite.smiteHeat(6, 'red').light, true)
  assert.equal(smite.smiteHeat(2, 'red').fill, '#450a0a')
  assert.equal(smite.smiteHeat(1, 'red').fill, '#ff1a1a')
  assert.equal(smite.smiteHeat(1, 'red').text, '#ffffff')
  assert.equal(smite.smiteHeat(12, 'amber').stroke, '#d97706')
  assert.notEqual(smite.smiteHeat(1, 'amber').fill, smite.smiteHeat(1, 'red').fill)
...
test('smite cake is a solid clock and does not reopen a long-press Word Bank dump', () => {
  ...
  const clock = styles.slice(styles.indexOf('g.logyq-smite-clock'), styles.indexOf('.logyq-smite-scar'))
