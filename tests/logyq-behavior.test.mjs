...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  const stayedOut = smite.smiteParentCommand(new Map([['a', 'red'], ['blank', 'red']]), 'a', 'red')
  assert.deepEqual(stayedOut.entries, [['a', 'amber'], ['blank', 'amber']])
  const kidsJoin = smite.smiteParentCommand(new Map([['blank', 'red']]), 'a', 'red')
  assert.equal(kidsJoin.action, 'rearm')
  assert.deepEqual(kidsJoin.entries, [['blank', 'amber'], ['a', 'amber']])
