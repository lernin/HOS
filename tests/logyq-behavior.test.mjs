...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  const fullRed = smite.smiteHeat(12, 'red')
  const fullAmber = smite.smiteHeat(12, 'amber')
  assert.equal(fullRed.stroke, '#fca5a5')
  assert.equal(fullAmber.stroke, '#fcd34d')
  assert.ok(fullRed.washOpacity <= 0.3 && fullRed.washOpacity > 0)
  assert.ok(fullAmber.washOpacity <= 0.3 && fullAmber.washOpacity > 0)
  assert.notEqual(smite.smiteHeat(12, 'red').stroke, smite.smiteHeat(8, 'red').stroke)
  assert.notEqual(smite.smiteHeat(8, 'red').stroke, smite.smiteHeat(4, 'red').stroke)
  assert.notEqual(smite.smiteHeat(4, 'red').stroke, smite.smiteHeat(1, 'red').stroke)
  assert.equal(smite.smiteHeat(1, 'red').stroke, '#ff2d2d')
  assert.equal(smite.smiteHeat(1, 'amber').stroke, '#ffb000')
  assert.ok(smite.smiteHeat(1, 'red').washOpacity <= 0.3)
  assert.notEqual(smite.smiteHeat(1, 'amber').wash, smite.smiteHeat(1, 'red').wash)
