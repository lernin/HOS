...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  const out = smite.smiteToggleParticipation(new Map([['a', 'red'], ['a1', 'red'], ['blank', 'red']]), 'a1', 'red')
  assert.equal(out.action, 'toggle')
  assert.equal(out.next, 'normal')
  assert.deepEqual(out.entries, [['a', 'red'], ['a1', 'normal'], ['blank', 'red']])
  const back = smite.smiteToggleParticipation(new Map(out.entries), 'a1', 'red')
  assert.equal(back.next, 'red')
  assert.deepEqual(back.entries, [['a', 'red'], ['a1', 'red'], ['blank', 'red']])
  const rootOut = smite.smiteToggleParticipation(new Map([['a', 'red'], ['a1', 'normal'], ['blank', 'amber']]), 'a', 'red')
  assert.equal(rootOut.next, 'normal')
  assert.deepEqual(rootOut.entries, [['a', 'normal'], ['a1', 'normal'], ['blank', 'amber']])
  const bankBack = smite.smiteToggleParticipation(new Map([['a', 'normal'], ['a1', 'amber']]), 'a', 'amber')
  assert.equal(bankBack.next, 'amber')
  assert.deepEqual(bankBack.entries, [['a', 'amber'], ['a1', 'amber']])
  assert.equal(smite.smiteToggleParticipation(new Map([['blank', 'red']]), 'a1', 'red').action, 'noop')
  ...
  assert.equal(smite.smiteCastReply(new Map([['a', 'red'], ['a1', 'normal']]), 'a', 'a1', 2, 3), 'toggle')
  assert.equal(smite.smiteCastReply(new Map([['a', 'red']]), 'a', 'a', 4, 2), 'toggle')
  assert.equal(smite.smiteCastReply(new Map([['a1', 'red']]), 'a', 'a', 2, 3), 'ignore')
...
test('smite cake is a solid clock and does not reopen a long-press Word Bank dump', () => {
  ...
  assert.match(v162, /function smiteToggleParticipation/)
  ...
  assert.match(mercyUp, /smiteToggleParticipation\(/)
  assert.doesNotMatch(mercyUp, /smiteParentCommand|smiteChildCommand/)
