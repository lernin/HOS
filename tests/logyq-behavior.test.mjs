...
function loadSmitePure() {
  ...
  assert.ok(start >= 0 && end > start)
  return new Function(`${source.slice(start, end)}; return { smiteZone, smiteCastDirection, smiteAffected, smiteNextMark, smiteRingFraction, smiteRefillMs, planSmiteCommit, smiteClockPath, smiteLinePhase, smiteHeat, smiteScarOpacity, smiteScarBlocked };`)()
}
...
test('smite cake zones, directions, marks, and the mercy ring', () => {
  ...
  assert.ok(ring.startsWith('M 70 0 '), 'the line starts at 12 o’clock')
  assert.ok(ring.includes('A 10 10 0 0 1 140 10'), 'from 12 it rounds clockwise onto the right side')
  ...
  assert.equal(smite.smiteLinePhase(1, ...box), 'full')
  assert.equal(smite.smiteLinePhase(smite.smiteRingFraction(15000), ...box), 'full')
  assert.equal(smite.smiteLinePhase(0.9, ...box), 'full')
  assert.equal(smite.smiteLinePhase(0.75, ...box), 'light')
  assert.equal(smite.smiteLinePhase(0.5, ...box), 'medium')
  assert.equal(smite.smiteLinePhase(0.22, ...box), 'dark')
  assert.equal(smite.smiteLinePhase(0.08, ...box), 'glow')
  assert.equal(smite.smiteLinePhase(0, ...box), 'glow')

  assert.equal(smite.smiteHeat('full', 'red').stroke, '#dc2626')
  assert.equal(smite.smiteHeat('light', 'red').stroke, '#fca5a5')
  assert.equal(smite.smiteHeat('medium', 'red').stroke, '#ef4444')
  assert.equal(smite.smiteHeat('dark', 'red').stroke, '#991b1b')
  assert.equal(smite.smiteHeat('dark', 'red').glow, 0)
  assert.equal(smite.smiteHeat('glow', 'red').stroke, '#ff2d2d')
  assert.ok(smite.smiteHeat('glow', 'red').glow > 0.5)
  assert.ok(smite.smiteHeat('glow', 'red').washOpacity <= 0.3)
  assert.equal(smite.smiteHeat('full', 'amber').stroke, '#d97706')
  assert.equal(smite.smiteHeat('glow', 'amber').stroke, '#ffb000')
  assert.notEqual(smite.smiteHeat('glow', 'amber').wash, smite.smiteHeat('glow', 'red').wash)
