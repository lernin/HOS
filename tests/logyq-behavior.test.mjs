...
test('preview gestures expose named tap/spawn seams and thresholds', () => {
  ...
  assert.match(gestures, /TAP_MOVE_PX: 9/)
  assert.match(gestures, /TAP_MAX_MS: 450/)
  assert.match(gestures, /SPAWN_AIM_PX: 22/)
  assert.match(gestures, /SPAWN_COMMIT_PX: 48/)
  assert.match(gestures, /SPAWN_MAX_MS: 850/)
  ...
  assert.match(ui, /bindSpawnGestures\(ui\.spawnPuck\)/)
  assert.match(ui, /bindCanvasGestures\(/)
  assert.doesNotMatch(ui, /ui\.spawnPuck\.addEventListener/)
  assert.equal((gestures.match(/addEventListener\('pointerdown', beginCanvasPointer/g) || []).length, 1)
  assert.equal((gestures.match(/addEventListener\('pointerdown', beginSpawnGesture/g) || []).length, 1)
})
