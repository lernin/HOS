...
test('LOGYQ phone smite cake parks a thumb, counts mercy, and banks only the amber path', async () => {
  ...
  assert.equal(toggled.find((card) => card.name === 'A1')?.wash, '#ffcc80')
  assert.equal(toggled.find((card) => card.name === 'A')?.clock, true)
  assert.equal(toggled.find((card) => card.name === 'A')?.wash, '#ffb8b8')
  assert.equal((await clocks())[0].stroke, '#ff0000')
  assert.equal((await edges()).some((edge) => edge.edge === '1'), false)
  assert.equal(await page.locator('.node-edit-input').count(), 0)
  ...
  const finishKids = await center('A')
  await touch('pointerdown', finishKids.x, finishKids.y, 93, finishKids.uid)
  await touch('pointermove', finishKids.x, finishKids.y + 70, 93, finishKids.uid)
  await touch('pointerup', finishKids.x, finishKids.y + 70, 93, finishKids.uid)
  await page.locator('[data-tool="undo"]').click()
