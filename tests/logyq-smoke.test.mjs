...
test('LOGYQ phone smite cake parks a thumb, counts mercy, and banks only the amber path', async () => {
  ...
  assert.deepEqual(family.map((card) => card.name), ['A'])
  assert.equal(family[0].clock, true)
  assert.equal(family[0].wash, '')
  assert.equal(family[0].glow, 0)
  assert.equal(family[0].faceStroke, 'none')
  ...
  assert.equal((await edges()).some((edge) => edge.edge === '1'), false)
  await page.evaluate(() => { window.LOGYQPreview.gestures.smite.mercy.remaining = 15000 })
  await page.waitForFunction(() => document.querySelector('path.logyq-smite-clock')?.getAttribute('stroke-dasharray') === 'none')
  ...
  assert.equal(toggled.find((card) => card.name === 'A1'), undefined)
  ...
  assert.equal(parallel.find((card) => card.name === 'B')?.wash, '')
  assert.equal(parallel.find((card) => card.name === 'A')?.clock, true)
  assert.equal(parallel.find((card) => card.name === 'A')?.wash, '')
  assert.equal(parallel.find((card) => card.name === 'A1'), undefined)
  ...
  const afterToggle = await clocks()
  assert.equal(afterToggle.find((clock) => clock.name === 'A1'), undefined)
  assert.equal(afterToggle.find((clock) => clock.name === 'A')?.stroke, '#ffa100')
  assert.equal(afterToggle.find((clock) => clock.name === 'B')?.stroke, '#ff0000')
  assert.equal((await edges()).find((edge) => edge.name === 'A1')?.edge || '', '')
  assert.equal(await page.evaluate(() => document.querySelectorAll('path.logyq-smite-clock').length), 2)
  assert.equal(await page.evaluate(() => window.LOGYQPreview.gestures.smite.mercies.length), 2)
  const overlapThumb = await park('middle', 91)
  const overlap = await center('A1')
  await touch('pointerdown', overlap.x, overlap.y, 92, overlap.uid)
  await touch('pointermove', overlap.x, overlap.y + 80, 92, overlap.uid)
  await touch('pointerup', overlap.x, overlap.y + 80, 92, overlap.uid)
  await touch('pointerup', overlapThumb.x, overlapThumb.y, 91)
  assert.equal(await page.evaluate(() => window.LOGYQPreview.gestures.smite.mercies.length), 2)
