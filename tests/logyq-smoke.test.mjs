...
test('LOGYQ phone smite cake parks a thumb, counts mercy, and banks only the amber path', async () => {
  ...
  assert.equal(parallel.find((card) => card.name === 'B')?.wash, '#f6b6b6')
  ...
  const split = await heats()
  assert.equal(split.find((card) => card.name === 'B')?.phase, 'l5')
  assert.equal(split.find((card) => card.name === 'B')?.wash, '#ff0000')
  assert.equal(split.find((card) => card.name === 'A')?.phase, 'l1')
  assert.equal(split.find((card) => card.name === 'A')?.wash, '#f6dfb6')
  await page.evaluate((uid) => {
    const mercy = window.LOGYQPreview.gestures.smite.mercies.find((item) => item.marks.has(uid))
  ...
  assert.equal(await page.evaluate(() => window.LOGYQPreview.gestures.smite.mercies.length), 2)
  assert.equal((await heats()).find((card) => card.name === 'A')?.wash, '#f6dfb6')
  ...
  assert.equal((await heats()).find((card) => card.name === 'A')?.clock, true)
  assert.equal((await heats()).find((card) => card.name === 'A')?.wash, '#f6dfb6')
  ...
  const both = await heats()
  assert.equal(await page.evaluate(() => window.LOGYQPreview.gestures.smite.mercies.length), 2)
  assert.equal(both.find((card) => card.name === 'Root')?.clock, true)
  assert.equal(both.find((card) => card.name === 'Root')?.wash, '#f6b6b6')
  assert.equal(both.find((card) => card.name === 'A')?.clock, true)
  assert.equal(both.find((card) => card.name === 'A')?.wash, '#f6dfb6')
