...
## Kid explanation selection — 2026-09-04

Goal: select one existing proposed kid explanation, explicitly choose confidence 0–3, then press Enter. The chosen text and score are sent in one guarded edit; the proposal heading, other candidates and nomination leave the active field. The existing audit preserves the prior proposal block. No selection or confidence is prefilled; changing the option resets confidence. Cancel does not write. Failed saves retain choices; retries reuse their request ID; duplicate submits are blocked. Ordinary explanations and unrecognized proposal formatting remain readable without being split or discarded. The kid explanation now appears between Technical Definition and Examples and in fresh copies, with normal confidence editing/discussion after selection.

HOS branch: `feat/kid-explanation-choice`. Production database dependency: Procedia branch `db/kid-explanation-choice`, migration `20260904064456_lab_thekonym_kid_explanation_edit.sql`. It adds only `kid_explanation` and `kid_explanation_confidence` to the existing password-checked update function. Existing password checks, expected values, row lock, retry IDs, audit and publication queue remain. The migration refuses a changed function checksum rather than overwriting concurrent work.

Validation: npm ci; npm run build; all 26 Node tests pass, including seven new parser/DOM interaction tests. No lint script is defined. The proposed SQL function passed a staging-only synthetic transaction rehearsal covering paired save at confidence 0, original-content audit, duplicate retries, stale conflicts, invalid scores, missing expected values, blank text, unsupported fields, wrong/null password and denied raw access. Fixtures were rolled back and their removal verified. Staging has no current viewer backend; the rehearsal uses isolated schemas with synthetic data and does not claim a full deployed backend test. Production function checksum and proposal text/confidence were confirmed unchanged.

Changed areas: viewer/types/copy, proposal parser, selection component/styles, assistant field support, tests and pinned jsdom development dependency. Other Lab tools and production data are untouched. No authenticated browser or phone test is claimed.

Release: obtain Ashley's explicit approval for the production save-function extension and merges/deployment. Apply the Procedia migration and verify, then merge/release HOS. Before the migration, a preview can display/select proposals but the old production RPC will reject saving these fields. Use the normal Lab password; open Thekonym viewer → Kyklonym → select a kid explanation → choose confidence → Enter. Check that only the chosen text remains and its confidence persists after reopening. Do not run this final save test on Kyklonym without Ashley choosing the explanation; use an approved synthetic record for release verification. No final explanation has been chosen for Ashley.

This supports terminology review; it does not complete the launch-critical child learning flow. Return to that critical path after this bounded release.
