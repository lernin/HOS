...
Six cleanup units are now applied to `/logiq-clean/` only:
...
- camera helpers with duplicate same-scope declarations until supersession is removed one declaration at a time;
...
Keyboard/listener inventory and legacy-map hygiene are established, and the first confidence-3 camera/moat dead helper has been removed from the clean candidate. Modular extraction remains gated until the high-confidence hygiene pass is complete. The next cleanup must again be a single verified unit, not a broad refactor.
